...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const fetchProviders = async () => {
            try {
                const res = await fetch("http://localhost:5000/api/movies/watch-providers");
            ...
            const fetchProfileData = async () => {
                ...
                try {
                    ...
                    if (currentProfile) {
                        ...
                        const trendingRes = await fetch("http://localhost:5000/api/movies/trending");
                        const trendingData = await trendingRes.json();
                        setTrending(trendingData.slice(0, 10));
                        setHeroMovie(trendingData[0]);

                        const topRatedRes = await fetch("http://localhost:5000/api/movies/top-rated");
    ...
    const handleUpdateProfile = async (id: string, updates: any) => {
        const token = localStorage.getItem("film_for_you_token");
        try {
            const response = await fetch(`http://localhost:5000/api/user/profiles/${id}`, {
                method: "PATCH",
                headers: {
            ...
            if (response.ok) {
                const profileRes = await fetch("http://localhost:5000/api/user/profiles", {
